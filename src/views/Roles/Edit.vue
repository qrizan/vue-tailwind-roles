<template>
    <form @submit.prevent="updateRole()"  v-if="checkPermission(['roles.edit'])">
            ...
            <div v-for="(permission) in permissions">
                <input 
                    ...
                    class="bg-gray-50 border-gray-300 
                        ...
                        :id=permission.id 
                        :checked="permissionsData.some((name) => name === permission.name ?? true)"
                    />
...
export default {
    ...
    setup() {
        ...
        const fetchDataRolesById = async() => {
            try {
                const response = await api.get(`/api/admin/roles/${route.params.id}`, {
                    headers: {
                        Authorization: `Bearer ${token}`,
                    },
                });
                ...
                if (response.data.success) {
                    ...
                    permissionsData.value = response.data.data.permissions.map((obj) => obj.name);
                } else {
        ...
        const updateRole = async () => {
            try {
                const response = await api.put(`/api/admin/roles/${route.params.id}`,
                    {
                        name: name.value,
